...
const EditDiscussion = () => {
  ...
  const getPostById = async () => {
    ...
    const kategoriString = response.data.kategori.join(", ");
    setKategori(kategoriString);

    if (response.data.url && response.data.url.length > 0) {
      const existingImages = response.data.url.map((imageUrl, index) => ({
        file: {
          name: response.data.images[index] || `image-${index}`,
          size: 0, // Anda bisa tambahkan logika untuk mendapatkan ukuran file jika perlu
        },
        preview: imageUrl,
        isExisting: true, // Tambahkan flag untuk membedakan gambar baru dan lama
      }));
  ...
  const updatePost = async (e) => {
    ...
    try {
      const token = localStorage.getItem("accessToken");
      const apiUrl = process.env.REACT_APP_URL_API;
      await axios.patch(`${apiUrl}/post/${id}`, formData, {
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  const handleRemoveImage = (index) => {
    ...
    // Jika gambar adalah gambar yang sudah ada, tambahkan ke daftar gambar yang dihapus
    if (image.isExisting) {
      setRemovedImages((prev) => [...prev, image.file.name]);
    } else {
      // Untuk gambar baru, cabut URL objek
                  ...
                  <div className="mt-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                      ...
                      >
                        ...
                        {/* Tampilkan tombol hapus untuk semua gambar dengan efek hover */}
                        <button
                          type="button"
                          ...
                          ${image.isExisting ? 'opacity-0 group-hover:opacity-100' : ''}`}
          ...
          <div className="flex">
            ...
            <div className="ml-3">
              <p className="text-sm text-gray-700 dark:text-white">
                Membuat postingan pertanyaan anda akan langsung tercatat dalam
                sistem. Pastikan buat pertanyaan sudah sesuai.
              </p>
            </div>
          </div>
        </div>
