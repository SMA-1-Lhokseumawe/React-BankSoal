...
const EditSoal = () => {
  ...
  const getSoalById = async () => {
    ...
    // Parse the soal if it's a JSON string (Delta format)
    let parsedSoal = response.data.soal;
    try {
      // If it's a JSON string, parse it
      if (typeof parsedSoal === 'string' && parsedSoal.includes('{"ops":')) {
        parsedSoal = JSON.parse(parsedSoal);
      }
    } catch (error) {
      console.error('Error parsing soal:', error);
    }
    ...
    setOptionE(response.data.optionE);
    setCorrectAnswer(response.data.correctAnswer);
    setKelasId(response.data.kelasId);
    setPelajaranId(response.data.pelajaranId);
  };

  const updateSoal = async (e) => {
    e.preventDefault();
    
    // Convert soal to JSON string if it's a Delta object
    const soalContent = typeof soal === 'object' 
      ? JSON.stringify(soal) 
      : soal;
          ...
          <div className="p-10">
            <form onSubmit={updateSoal} className="space-y-8">
              <div className="bg-blue-50 dark:bg-gray-700 p-6 rounded-lg">
                ...
                <ReactQuill
                  ref={quillRef}
                  value={soal || ""} // Provide a fallback empty string
                  onChange={(content, delta, source, editor) => {
                    // Use editor.getContents() to get the full Delta object
                    setSoal(editor.getContents());
                  }}
                  modules={modules}
              ...
              <div className="flex items-center justify-between pt-8 px-6">
                ...
                >
                  {isSubmitting ? "Menyimpan..." : "Update Soal"}
                </button>
              </div>
            </form>
          </div>
