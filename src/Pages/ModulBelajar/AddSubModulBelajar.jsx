...
const AddSubModulBelajar = () => {
  ...
  useEffect(() => {
    ...
    if (token) {
      ...
      if (location.state && location.state.modulId) {
        ...
        console.log("Tidak ada modulId yang diterima");
      }
  ...
  // Tambahkan field modulId yang sudah terisi dan disabled
  const formSection = (
    <>
      {/* Field modulId yang sudah terisi */}
  ...
  const saveSubModul = async (e) => {
    ...
    try {
      ...
      navigate("/list-sub-modul-belajar");
    } catch (error) {
      console.error(
        "Error:",
      ...
      <div className="max-w-full mx-auto">
        <div className="bg-white dark:bg-secondary-dark-bg rounded-xl shadow-lg overflow-hidden mx-8">
          ...
          <div className="py-4 px-6" style={{ backgroundColor: currentColor }}>
            <h1 className="text-xl font-bold text-white">Tambah Sub Modul Baru</h1>
          </div>
